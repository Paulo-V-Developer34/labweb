<?php
        ...
        if(isset($_POST["tecnico_matricula"])){
            ...
            $sql = "SELECT * FROM tecnicos WHERE matricula = ".$_POST["tecnico_matricula"];
            ...
            foreach($resultado as $linha) {
                ...
                foreach($resultado_2 as $linha_2) {
                    ...
                    $retorno .= '<input class="w3-input w3-border w3-margin-bottom w3-light-grey" type="text" name = "nomeTecExcluir" readonly value = '.$linha_2['nome'].'>';
                    $retorno .= '</div>';
                }

                $retorno .= '<div class="w3-row-padding" style="margin:0 -8px;">';
                $retorno .= '<label><b>CPF</b></label>';
                $retorno .= '<input class="w3-input w3-border w3-margin-bottom w3-light-grey" type="text" name = "cpfTecExcluir" readonly value = '.$linha['cpf'].'>';
                $retorno .= '</div>';

                $retorno .= '<div class="w3-row-padding" style="margin:0 -8px;">';
                $retorno .= '<label><b>Matrícula</b></label>';
                $retorno .= '<input class="w3-input w3-border w3-margin-bottom w3-light-grey" type="text" name = "matriculaTecExcluir" readonly value = '.$linha['matricula'].'>';
                $retorno .= '</div>';
