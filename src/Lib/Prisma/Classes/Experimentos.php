...
class Experimentos implements IModel
{
    public $numerolote;
    public $dataexperimento;
    public $datavalidadeexperimento;
    public $peso;
    public $volume;
    public $presencasm;
    public $temperaturalote;
    public $ph;
    public $condicaoexperimento;
    public $obsexperimento;
    public $matriculatecnico;
    public $tecnicos;
    public $_col;
    ...
    public function __construct($pdo, $data = null)
    {
        ...
        $this->_fields = array(
            ...
            'datavalidadeexperimento' =>
            array(
                'name' => 'datavalidadeexperimento',
                'type' => 'DateTime',
                'isNullable' => '',
                'isPrimaryKey' => '',
                'decorators' =>
            ...
            array(
                ...
                array (
                    ...
                    array (
                      ...
                      'onUpdate' => 'NoAction',
                      'type' => 'ManyToOne',
                      'tableName' => 'experimentos',
                      'tableModelName' => 'experimentos',
        ...
        $this->_modelName = 'Experimentos';
        $this->_fieldsOnly = ['numerolote', 'dataexperimento', 'datavalidadeexperimento', 'peso', 'volume', 'presencasm', 'temperaturalote', 'ph', 'condicaoexperimento', 'obsexperimento', 'matriculatecnico'];
        $this->_fieldsRelated = ['tecnicos'];

        $this->_col = new class()
        {
            public function __construct(
                public readonly string $numerolote = 'numerolote',
                public readonly string $dataexperimento = 'dataexperimento',
                public readonly string $datavalidadeexperimento = 'datavalidadeexperimento',
                public readonly string $peso = 'peso',
                public readonly string $volume = 'volume',
                public readonly string $presencasm = 'presencasm',
                public readonly string $temperaturalote = 'temperaturalote',
                public readonly string $ph = 'ph',
                public readonly string $condicaoexperimento = 'condicaoexperimento',
                public readonly string $obsexperimento = 'obsexperimento',
                public readonly string $matriculatecnico = 'matriculatecnico',
        ...
        if ($data) {
            $this->numerolote = $data['numerolote'] ?? null;
            $this->dataexperimento = $data['dataexperimento'] ?? null;
            $this->datavalidadeexperimento = $data['datavalidadeexperimento'] ?? null;
            $this->peso = $data['peso'] ?? null;
            $this->volume = $data['volume'] ?? null;
            $this->presencasm = $data['presencasm'] ?? null;
            $this->temperaturalote = $data['temperaturalote'] ?? null;
            $this->ph = $data['ph'] ?? null;
            $this->condicaoexperimento = $data['condicaoexperimento'] ?? null;
            $this->obsexperimento = $data['obsexperimento'] ?? null;
            $this->matriculatecnico = $data['matriculatecnico'] ?? null;
    ...
    protected function includeTecnicos(array $items, array $selectedFields = [], array $includeSelectedFields = [], bool $format = false) 
    {
        ...
        $tableName = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"experimentos\"" : "`experimentos`";
        $tableModelName = 'Experimentos';
        $tablePrimaryKey = '';
        ...
        $foreignKeyIds = array_column($items, 'matriculatecnico');
    ...
    protected function connectTecnicos(string $relationName, array|bool $connectData, string $lastInsertId, string $connectType = 'connect')
    {
        $dbType = $this->_dbType;
        $quotedTableName = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"tecnicos\"" : "`tecnicos`";
        $tableName = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"experimentos\"" : "`experimentos`";
        $modelName = 'Tecnicos';
        $tablePrimaryKey = '';
        $tableForeignKey = 'matriculatecnico';
        $relationType = 'ManyToOne';
        $typeOfTableRelation = 'relation';
        $tablePrimaryKeyQuoted = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"\"" : "``";
        $relatedPrimaryKey = 'matricula';
        $primaryKeyQuoted = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"matricula\"" : "`matricula`";
        $foreignKeyQuoted = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"matriculatecnico\"" : "`matriculatecnico`";
        ...
        if (!empty($foundUnique) && $connectType === 'disconnect') {
            try {
                ...
                throw new \Exception("Error disconnecting tecnicos: " . $e->getMessage());
            }
        }
    }
    ...
    {
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required when creating a new Experimentos.");
        }
        ...
        $requiredFieldsMap = [
            'numerolote' => 'Int',
            'dataexperimento' => 'DateTime',
            'datavalidadeexperimento' => 'DateTime',
            'peso' => 'Float',
            'volume' => 'Float',
            'presencasm' => 'Boolean',
            'temperaturalote' => 'Float',
            'ph' => 'Float',
            'condicaoexperimento' => 'String',
            'obsexperimento' => 'String',
            'matriculatecnico' => 'Int',
        ];
        foreach ($requiredFieldsMap as $fieldName => $fieldType) {
            if (!isset($data['data'][$fieldName])) {
                throw new \Exception("The '$fieldName' field of type '$fieldType' is required when creating a new Experimentos.");
            }
        }
        ...
        $bindings = [];
        $dbType = $this->_dbType;
        $quotedTableName = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"experimentos\"" : "`experimentos`";
                ...
                foreach ($data as $relationName => $relationDataName) {
                    ...
                    if (in_array($relationName, $relationNames)) {
                        ...
                        if (isset($relationDataName['create']) || isset($relationDataName['createMany']) || isset($relationDataName['connect']) || isset($relationDataName['connectOrCreate'])) {
                            throw new \Exception("The relation name '$relationName' is not defined in the Experimentos model.");
                        }
                    }
                }
            }
    ...
    public function findUnique(array $criteria, bool $format = false): array | object
    {
        ...
        if (!$whereHasUniqueKey) {
            throw new \Exception("No valid 'where' conditions provided for finding a unique record in Experimentos.");
        }
        ...
        foreach ($includes as $relation => $relationInclude) {
            if ($relationInclude) {
                ...
                if (method_exists($this, $includeMethodName)) {
                    ...
                    throw new \Exception("The '$relation' does not exist, in the Experimentos model.");
                }
            }
        }
    ...
    {
        if (!isset($data['where'])) {
            throw new \Exception("The 'where' key is required in the update Experimentos.");
        }
        ...
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required in the update Experimentos.");
        }
        ...
        foreach ($requiredFieldsMap as $fieldName => $fieldType) {
            if (isset($data['data'][$fieldName]) && empty($data['data'][$fieldName])) {
                throw new \Exception("The '$fieldName' field of type '$fieldType' is required in Experimentos Model.");
            }
        }
    ...
    public function delete(array $criteria, bool $format = false): array | object
    {
        ...
        try {
            ...
            return $affectedRows ? $deletedRow : ['modelName' => 'Experimentos', 'cause' => 'Record to delete does not exist.'];
        } catch (\Exception $e) {
            $this->_pdo->rollBack();
            throw $e;
        }
    ...
    public function aggregate(array $operation, bool $format = false): array | object
    {
        ...
        $quotedTableName = $this->_dbType == 'pgsql' ? "\"experimentos\"" : "`experimentos`";
        $conditions = [];
        $bindings = [];
    ...
    {
        if (!isset($data['where'])) {
            throw new \Exception("The 'where' key is required in the updateMany Experimentos.");
        }
        ...
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required in the updateMany Experimentos.");
        }
...
";
    ...
    public function count(array $criteria = [], bool $format = false): int | array | object
    {
        ...
        $select = $criteria['select'] ?? [];
        $dbType = $this->_dbType;
        $quotedTableName = $dbType === 'pgsql' ? "\"experimentos\"" : "`experimentos`";
