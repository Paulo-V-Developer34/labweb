...
class Clientes implements IModel
{
    public $numero;
    public $empresa;
    ...
    public function __construct($pdo, $data = null)
    {
        ...
        $this->_modelName = 'Clientes';
        $this->_fieldsOnly = ['numero', 'empresa', 'statususuarios', 'cpf'];
        $this->_fieldsRelated = ['pessoas'];

        $this->_col = new class()
        {
            public function __construct(
                public readonly string $numero = 'numero',
                public readonly string $empresa = 'empresa',
                public readonly string $statususuarios = 'statususuarios',
        ...
        if ($data) {
            ...
            $this->statususuarios = $data['statususuarios'] ?? null;
    ...
    protected function includePessoas(array $items, array $selectedFields = [], array $includeSelectedFields = [], bool $format = false) 
    {
        ...
        $tableName = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"clientes\"" : "`clientes`";
        $tableModelName = 'Clientes';
        $tablePrimaryKey = '';
    ...
    {
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required when creating a new Clientes.");
        }
        ...
        foreach ($requiredFieldsMap as $fieldName => $fieldType) {
            if (!isset($data['data'][$fieldName])) {
                throw new \Exception("The '$fieldName' field of type '$fieldType' is required when creating a new Clientes.");
            }
        }
        ...
        $bindings = [];
        $dbType = $this->_dbType;
        $quotedTableName = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"clientes\"" : "`clientes`";
                ...
                foreach ($data as $relationName => $relationDataName) {
                    ...
                    if (in_array($relationName, $relationNames)) {
                        ...
                        if (isset($relationDataName['create']) || isset($relationDataName['createMany']) || isset($relationDataName['connect']) || isset($relationDataName['connectOrCreate'])) {
                            throw new \Exception("The relation name '$relationName' is not defined in the Clientes model.");
                        }
                    }
                }
            }
    ...
    public function findUnique(array $criteria, bool $format = false): array | object
    {
        ...
        if (!$whereHasUniqueKey) {
            throw new \Exception("No valid 'where' conditions provided for finding a unique record in Clientes.");
        }
        ...
        foreach ($includes as $relation => $relationInclude) {
            if ($relationInclude) {
                ...
                if (method_exists($this, $includeMethodName)) {
                    ...
                    throw new \Exception("The '$relation' does not exist, in the Clientes model.");
                }
            }
        }
    ...
    {
        if (!isset($data['where'])) {
            throw new \Exception("The 'where' key is required in the update Clientes.");
        }
        ...
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required in the update Clientes.");
        }
        ...
        foreach ($requiredFieldsMap as $fieldName => $fieldType) {
            if (isset($data['data'][$fieldName]) && empty($data['data'][$fieldName])) {
                throw new \Exception("The '$fieldName' field of type '$fieldType' is required in Clientes Model.");
            }
        }
    ...
    public function delete(array $criteria, bool $format = false): array | object
    {
        ...
        try {
            ...
            return $affectedRows ? $deletedRow : ['modelName' => 'Clientes', 'cause' => 'Record to delete does not exist.'];
        } catch (\Exception $e) {
            $this->_pdo->rollBack();
            throw $e;
        }
    ...
    public function aggregate(array $operation, bool $format = false): array | object
    {
        ...
        $quotedTableName = $this->_dbType == 'pgsql' ? "\"clientes\"" : "`clientes`";
        $conditions = [];
        $bindings = [];
    ...
    {
        if (!isset($data['where'])) {
            throw new \Exception("The 'where' key is required in the updateMany Clientes.");
        }
        ...
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required in the updateMany Clientes.");
        }
...
";
    ...
    public function count(array $criteria = [], bool $format = false): int | array | object
    {
        ...
        $select = $criteria['select'] ?? [];
        $dbType = $this->_dbType;
        $quotedTableName = $dbType === 'pgsql' ? "\"clientes\"" : "`clientes`";
