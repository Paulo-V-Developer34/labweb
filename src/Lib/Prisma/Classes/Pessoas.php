...
class Pessoas implements IModel
{
    public $cpf;
    public $nome;
    public $telefonefixo;
    ...
    public function __construct($pdo, $data = null)
    {
        ...
        $this->_fields = array(
            ...
            array(
                ...
                array (
                    ...
                    array (
                      'fromModel' => 'pessoas',
                      'fromModelTableName' => 'pessoas',
                      'fromField' => 'administradores',
                      'toModel' => 'administradores',
                      'toField' => 'pessoas',
                      'type' => 'OneToMany',
                      'fields' => 
                      array (
        ...
        $this->_modelName = 'Pessoas';
        $this->_fieldsOnly = ['cpf', 'nome', 'telefonefixo', 'telefonecelular', 'email', 'rua', 'bairro', 'cidade', 'estado', 'senha', 'tipo_usuario'];
        $this->_fieldsRelated = ['administradores', 'clientes', 'tecnicos'];
        ...
        $this->_col = new class()
        {
            public function __construct(
                ...
                public readonly string $telefonefixo = 'telefonefixo',
                public readonly string $telefonecelular = 'telefonecelular',
                public readonly string $email = 'email',
                public readonly string $rua = 'rua',
                public readonly string $bairro = 'bairro',
                public readonly string $cidade = 'cidade',
                public readonly string $estado = 'estado',
                public readonly string $senha = 'senha',
                public readonly string $tipo_usuario = 'tipo_usuario',
                public readonly string $administradores = 'administradores',
                public readonly string $clientes = 'clientes',
                public readonly string $tecnicos = 'tecnicos',
            ) {
            }
        };
        ...
        if ($data) {
            ...
            $this->telefonefixo = $data['telefonefixo'] ?? null;
            $this->telefonecelular = $data['telefonecelular'] ?? null;
            ...
            $this->administradores = new Administradores($this->_pdo, $data['administradores'] ?? null);
            $this->clientes = new Clientes($this->_pdo, $data['clientes'] ?? null);
            $this->tecnicos = new Tecnicos($this->_pdo, $data['tecnicos'] ?? null);
        }
    }

    protected function includeAdministradores(array $items, array $selectedFields = [], array $includeSelectedFields = [], bool $format = false) 
    {
        if (empty($items)) {
            return $items;
        }
        ...
        $tableName = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"pessoas\"" : "`pessoas`";
        $modelName = 'administradores';
        $relationName = 'administradores';
        ...
        $instanceModelName = new administradores($this->_pdo);
    ...
    protected function connectAdministradores(string $relationName, array $connectData, string $lastInsertId, string $connectType = 'connect')
    {
        $dbType = $this->_dbType;
        $quotedTableName = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"pessoas\"" : "`pessoas`";
        $tableName = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"pessoas\"" : "`pessoas`";
        ...
        $relationModel = new administradores($this->_pdo);
    ...
    protected function includeClientes(array $items, array $selectedFields = [], array $includeSelectedFields = [], bool $format = false) 
    {
        if (empty($items)) {
            return $items;
        }
        ...
        $instanceModelName = new clientes($this->_pdo);
        foreach ($items as &$item) {
            if (!isset($item[$primaryKey])) {
                $item[$relationName] = [];
                continue;
    ...
    protected function connectClientes(string $relationName, array $connectData, string $lastInsertId, string $connectType = 'connect')
    {
        $dbType = $this->_dbType;
        $quotedTableName = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"pessoas\"" : "`pessoas`";
        $tableName = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"pessoas\"" : "`pessoas`";
        ...
        $relationModel = new clientes($this->_pdo);
    ...
    protected function includeTecnicos(array $items, array $selectedFields = [], array $includeSelectedFields = [], bool $format = false) 
    {
        if (empty($items)) {
            return $items;
        }
        ...
        $instanceModelName = new tecnicos($this->_pdo);
        foreach ($items as &$item) {
            if (!isset($item[$primaryKey])) {
                $item[$relationName] = [];
                continue;
    ...
    protected function connectTecnicos(string $relationName, array $connectData, string $lastInsertId, string $connectType = 'connect')
    {
        $dbType = $this->_dbType;
        $quotedTableName = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"pessoas\"" : "`pessoas`";
        $tableName = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"pessoas\"" : "`pessoas`";
        ...
        $relationModel = new tecnicos($this->_pdo);
    ...
    {
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required when creating a new Pessoas.");
        }
        ...
        foreach ($requiredFieldsMap as $fieldName => $fieldType) {
            if (!isset($data['data'][$fieldName])) {
                throw new \Exception("The '$fieldName' field of type '$fieldType' is required when creating a new Pessoas.");
            }
        }
        ...
        $data = $data['data'];
        $relationNames = ['administradores', 'clientes', 'tecnicos'];
                ...
                foreach ($data as $relationName => $relationDataName) {
                    ...
                    if (in_array($relationName, $relationNames)) {
                        ...
                        if (isset($relationDataName['create']) || isset($relationDataName['createMany']) || isset($relationDataName['connect']) || isset($relationDataName['connectOrCreate'])) {
                            throw new \Exception("The relation name '$relationName' is not defined in the Pessoas model.");
                        }
                    }
                }
            }
    ...
    public function findUnique(array $criteria, bool $format = false): array | object
    {
        ...
        if (!$whereHasUniqueKey) {
            throw new \Exception("No valid 'where' conditions provided for finding a unique record in Pessoas.");
        }
        ...
        foreach ($includes as $relation => $relationInclude) {
            if ($relationInclude) {
                ...
                if (method_exists($this, $includeMethodName)) {
                    ...
                    throw new \Exception("The '$relation' does not exist, in the Pessoas model.");
                }
            }
        }
    ...
    {
        if (!isset($data['where'])) {
            throw new \Exception("The 'where' key is required in the update Pessoas.");
        }
        ...
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required in the update Pessoas.");
        }
        ...
        foreach ($requiredFieldsMap as $fieldName => $fieldType) {
            if (isset($data['data'][$fieldName]) && empty($data['data'][$fieldName])) {
                throw new \Exception("The '$fieldName' field of type '$fieldType' is required in Pessoas Model.");
            }
        }
                ...
                foreach ($data as $relationName => $relationDataName) {
                    ...
                    if (in_array($relationName, $relationNames)) {
                        ...
                        if (isset($relationDataName['create']) && is_array($relationDataName['create'])) {
                            ...
                            if ($checkArrayContentType !== ArrayType::Associative) {
                                throw new \Exception("To create many records, use an associative array with the field names as keys and the data to create as values in tecnicos model. ['createMany' => [['name' => 'someName'], ['name' => 'someOtherName']]");
                            }
                    ...
                    if (isset($relationDataName['connect']) || isset($relationDataName['connectOrCreate']) || isset($relationDataName['disconnect']) || isset($relationDataName['update']) || isset($relationDataName['updateMany'])) {
                        ...
                        if (isset($relationDataName['connect'])) {
                            ...
                            if (isset($relationDataName['connect']) && $checkArrayContentType !== ArrayType::Value) {
                                throw new \Exception("The 'connect' key must be an associative array with the field names as keys and the data to create as values related tecnicos model. example: ['connect' => ['id' => 'someId']]");
                            }
                        }
                        ...
                        if (isset($relationDataName['connectOrCreate'])) {
                            ...
                            if (isset($relationDataName['connectOrCreate']) && $checkArrayContentType !== ArrayType::Associative) {
                                throw new \Exception("The 'connectOrCreate' key must be an associative array with the field names as keys and the data to create as values related tecnicos model. example: ['connectOrCreate' => ['where' => ['id' => 'someId'], 'create' => ['name' => 'someName']]");
                            }
                        }
                        ...
                        if (isset($relationDataName['disconnect'])) {
                            ...
                            if (!is_bool($disconnectData)) {
                                ...
                                if (isset($relationDataName['disconnect']) && $checkArrayContentType !== ArrayType::Value) {
                                    throw new \Exception("The 'disconnect' key must be an associative array with the field names as keys and the data to create as values related tecnicos model. example: ['disconnect' => ['id' => 'someId']]");
                                }
                            }
                        }
    ...
    public function delete(array $criteria, bool $format = false): array | object
    {
        ...
        try {
            ...
            return $affectedRows ? $deletedRow : ['modelName' => 'Pessoas', 'cause' => 'Record to delete does not exist.'];
        } catch (\Exception $e) {
            $this->_pdo->rollBack();
            throw $e;
        }
    ...
    public function aggregate(array $operation, bool $format = false): array | object
    {
        ...
        $quotedTableName = $this->_dbType == 'pgsql' ? "\"pessoas\"" : "`pessoas`";
        $conditions = [];
        $bindings = [];
    ...
    {
        if (!isset($data['where'])) {
            throw new \Exception("The 'where' key is required in the updateMany Pessoas.");
        }
        ...
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required in the updateMany Pessoas.");
        }
...
";
    ...
    public function count(array $criteria = [], bool $format = false): int | array | object
    {
        ...
        $select = $criteria['select'] ?? [];
        $dbType = $this->_dbType;
        $quotedTableName = $dbType === 'pgsql' ? "\"pessoas\"" : "`pessoas`";
