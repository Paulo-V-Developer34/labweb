 ...
 * @property Experimentos $experimentos
 * @property Pessoas $pessoas
 * @property Tecnicos $tecnicos
 */
class Prisma
{
...
class DatabaseConnection
{
    ...
    private function __construct()
    {
        // Lazy initialization of the PDO connection
        $this->initializePDO();
    }
    ...
    {
        // Lazy initialization to ensure PDO is only created when needed
        $this->initializePDO();
        return $this->_pdo;
    }
    ...
    {
        if ($this->_pdo === null) {
            // Check if the PDO instance is already initialized
...
class ExtendedPDO extends \PDO
{
    ...
    // To keep track of active statements
    private $activeStatements = [];
    ...
    public function beginTransaction(): bool
    {
        if ($this->transactionDepth == 0) {
            ...
            // Finalize any active statements before creating a savepoint
    ...
    public function commit(): bool
    {
        ...
        if ($this->transactionDepth == 0) {
            if ($this->rollbackRequested) {
                ...
                // Finalize all active statements before committing
                $this->finalizeStatements();
                parent::commit();
            }
            ...
            if ($this->rollbackRequested) {
                ...
                // Finalize all active statements before releasing the savepoint
                $this->finalizeStatements();
    ...
    // Method to finalize all active statements
    private function finalizeStatements(): void
    {
        foreach ($this->activeStatements as $stmt) {
            $stmt->closeCursor(); // Finalize each statement
        }
        $this->activeStatements = []; // Clear the list of active statements
    }
}