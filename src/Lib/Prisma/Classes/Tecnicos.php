...
class Tecnicos implements IModel
{
    public $matricula;
    public $cargo;
    public $registro;
    ...
    public function __construct($pdo, $data = null)
    {
        ...
        $this->_fields = array(
            ...
            array(
                ...
                array (
                    ...
                    array (
                      'fromModel' => 'tecnicos',
                      'fromModelTableName' => 'tecnicos',
                      'fromField' => 'experimentos',
                      'toModel' => 'experimentos',
                      'toField' => 'tecnicos',
                      'type' => 'OneToMany',
                      'fields' => 
                      array (
        ...
        $this->_modelName = 'Tecnicos';
        $this->_fieldsOnly = ['matricula', 'cargo', 'registro', 'dataadmissao', 'datarecisao', 'statustecnico', 'cpf'];
        $this->_fieldsRelated = ['experimentos', 'pessoas'];

        $this->_col = new class()
        {
            public function __construct(
                public readonly string $matricula = 'matricula',
                public readonly string $cargo = 'cargo',
                public readonly string $registro = 'registro',
                public readonly string $dataadmissao = 'dataadmissao',
                public readonly string $datarecisao = 'datarecisao',
                public readonly string $statustecnico = 'statustecnico',
                public readonly string $cpf = 'cpf',
                public readonly string $experimentos = 'experimentos',
                public readonly string $pessoas = 'pessoas',
            ) {
            }
        };
        ...
        if ($data) {
            ...
            $this->datarecisao = $data['datarecisao'] ?? null;
            $this->statustecnico = $data['statustecnico'] ?? null;
            $this->cpf = $data['cpf'] ?? null;
            $this->experimentos = new Experimentos($this->_pdo, $data['experimentos'] ?? null);
            $this->pessoas = new Pessoas($this->_pdo, $data['pessoas'] ?? null);
        }
    }

    protected function includePessoas(array $items, array $selectedFields = [], array $includeSelectedFields = [], bool $format = false) 
    {
        if (empty($items)) {
            return $items;
        }
        ...
        $dbType = $this->_dbType;
        $quotedTableName = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"pessoas\"" : "`pessoas`";
        $tableName = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"tecnicos\"" : "`tecnicos`";
        $tableModelName = 'Tecnicos';
        $tablePrimaryKey = '';
        $tablePrimaryKeyQuoted = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"\"" : "``";
        $modelName = 'Pessoas';
        $relationName = 'pessoas';
        $relationType = 'ManyToOne';
        $typeOfTableRelation = 'relation';
        $foreignKeyIds = array_column($items, 'cpf');
        $primaryKey = 'cpf';
        $foreignKey = 'cpf';
        $primaryKeyQuoted = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"cpf\"" : "`cpf`";
        $foreignKeyQuoted = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"cpf\"" : "`cpf`";
        ...
        if (!empty($foreignKeyIds)) {
            $instanceModelName = new Pessoas($this->_pdo);
            foreach ($items as &$item) {
                ...
                $relatedRecords = $instanceModelName->findUnique($mergeQuery, $format);
    ...
    protected function connectPessoas(string $relationName, array|bool $connectData, string $lastInsertId, string $connectType = 'connect')
    {
        $dbType = $this->_dbType;
        $quotedTableName = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"pessoas\"" : "`pessoas`";
        $tableName = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"tecnicos\"" : "`tecnicos`";
        ...
        if ($typeOfTableRelation === 'relation' && $relationType === 'OneToMany' && $connectType === 'updateMany') {
            throw new \Exception("Error connecting $modelName: relation does not support 'updateMany' use 'update' instead");
        }
        ...
        $relationModel = new Pessoas($this->_pdo);
        $where = $connectData['where'] ?? $connectData;

        if ($connectType === 'create') {
            if ($typeOfTableRelation === 'relation') {
                $checkArrayContentType = Utility::checkArrayContents($connectData);

                if ($checkArrayContentType !== ArrayType::Value) {
                    throw new \Exception(
                        "Invalid data format for creating a new record in the '{$relationName}' model. 
                        When using the 'create' operation, the provided value must be a single array with field names as keys and corresponding data as values. 
                        Example: ['create' => ['name' => 'someName']]."
                    );
                }
            }
        ...
        if ($connectType === 'update') {
            if ($typeOfTableRelation === 'relation') {
                ...
                if ($checkArrayContentType !== ArrayType::Value) {
                    throw new \Exception(
                        "Invalid data format for updating a record in the '{$relationName}' model. 
                        When using the 'update' operation, the value must be a single array with field names as keys and the corresponding data as values. 
                        Example: ['update' => ['name' => 'someName']]."
                    );
                }
            }
        ...
        if (empty($foundUnique) && $connectType === 'connect') {
            throw new \Exception("Error connecting '{$modelName}': No matching record found for the provided connection data.");
        }
        ...
        if (!empty($foundUnique) && $connectType === 'connect') {
            try {
                $whereClause = ['where' => [$tablePrimaryKey => $lastInsertId], 'data' => [$tableForeignKey => $foundUnique[$relatedPrimaryKey]]];
                $this->update($whereClause);
            } catch (\Exception $e) {
                throw new \Exception("Error connecting records: " . $e->getMessage());
            }
        }
        ...
        if (!empty($foundUnique) && $connectType === 'disconnect') {
            try {
                ...
                throw new \Exception("Error disconnecting pessoas: " . $e->getMessage());
            }
        }
    }

    protected function includeExperimentos(array $items, array $selectedFields = [], array $includeSelectedFields = [], bool $format = false) 
    {
        if (empty($items)) {
            return $items;
        }
        ...
        $dbType = $this->_dbType;
        $quotedTableName = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"tecnicos\"" : "`tecnicos`";
        $tableName = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"tecnicos\"" : "`tecnicos`";
        $modelName = 'experimentos';
        $relationName = 'experimentos';
        $foreignKeyIds = array_column($items, 'matricula');
        $relationType = 'OneToMany';
        $typeOfTableRelation = 'inverse';
        $primaryKey = 'matricula';
        $foreignKey = 'matriculatecnico';
        $primaryKeyQuoted = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"matricula\"" : "`matricula`";
        $foreignKeyQuoted = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"matriculatecnico\"" : "`matriculatecnico`";
        $foreignKeyIds = array_filter($foreignKeyIds); // Filter out any empty values
        $foreignKeyIds = array_unique($foreignKeyIds);

        $instanceModelName = new experimentos($this->_pdo);
        foreach ($items as &$item) {
    ...
    protected function connectExperimentos(string $relationName, array $connectData, string $lastInsertId, string $connectType = 'connect')
    {
        $dbType = $this->_dbType;
        $quotedTableName = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"tecnicos\"" : "`tecnicos`";
        $tableName = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"tecnicos\"" : "`tecnicos`";
        $modelName = 'experimentos';
        $tablePrimaryKey = '';
        $relatedForeignKey = 'matriculatecnico';
        $relationType = 'OneToMany';
        $typeOfTableRelation = 'inverse';
        $primaryKeyQuoted = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"matricula\"" : "`matricula`";
        $foreignKeyQuoted = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"matriculatecnico\"" : "`matriculatecnico`";
        ...
        $relationModel = new experimentos($this->_pdo);

        if ($connectType === 'create') {
            // Check if the provided data is for single or multiple record creation
            $isMultiple = is_array($connectData) && array_keys($connectData) === range(0, count($connectData) - 1);

            if ($isMultiple) {
                if ($typeOfTableRelation === 'inverse') {
                    if ($relationType !== 'OneToMany') {
                        throw new \Exception("Error creating records for '{$modelName}': The 'create' operation is only supported for OneToMany relations when dealing with multiple records.");
                    }
                    ...
                    foreach ($dataToUpdate as $data) {
                        $relationModel->create(['data' => $data]);
                    }
                }
            } else {
        ...
        if ($connectType === 'createMany') {
            if ($typeOfTableRelation === 'inverse' && $relationType !== 'OneToMany') {
                throw new \Exception("Error creating multiple records for '{$modelName}': The 'createMany' operation is only supported for OneToMany relations when dealing with multiple records.");
            }
        ...
        if ($connectType === 'update' || $connectType === 'updateMany') {
            if ($typeOfTableRelation === 'inverse' && $relationType === 'OneToMany' && !isset($connectData['where']) && !isset($connectData['data'])) {
                throw new \Exception("Error updating '{$modelName}': OneToMany relation requires both 'where' and 'data' keys to be specified.");
            }
            ...
            try {
                ...
                throw new \Exception("Error updating '{$modelName}': " . $e->getMessage());
            }
        }

        $isMultiple = Utility::checkArrayContents($connectData) === ArrayType::Associative;
        $foundUnique = null;
        if (!$isMultiple) {
            $where = isset($connectData['where']) ? $connectData['where'] : ['where' => $connectData];
        ...
        if (!empty($connectData) && $connectType === 'connect') {
            try {
                if ($isMultiple) {
                    if ($typeOfTableRelation === 'inverse') {
                        if ($relationType !== 'OneToMany') {
                            throw new \Exception("Error connecting '{$modelName}': The 'connect' operation is only supported for OneToMany relations when dealing with multiple records.");
                        }

                        foreach ($connectData as $idx => $data) {
                            $whereClause = ['where' => $data, 'data' => [$relatedForeignKey => $lastInsertId]];
                            $relationModel->update($whereClause);
                        }
                    }
                } else {
                    if (!empty($foundUnique)) {
                        $whereClause = ['where' => [$tablePrimaryKey => $foundUnique[$tablePrimaryKey]], 'data' => [$relatedForeignKey => $lastInsertId]];
                        $relationModel->update($whereClause);
                    }
                }
            } catch (\Exception $e) {
        ...
        if (!empty($connectData) && $connectType === 'disconnect') {
            try {
                if ($isMultiple) {
                    if ($typeOfTableRelation === 'inverse') {
                        if ($relationType !== 'OneToMany') {
                            throw new \Exception("Error disconnecting '{$modelName}': The 'disconnect' operation is only supported for OneToMany relations when dealing with multiple records.");
                        }

                        foreach ($connectData as $idx => $data) {
                            $whereClause = ['where' => $data, 'data' => [$relatedForeignKey => null]];
                            $relationModel->update($whereClause);
                        }
                    }
                } else {
                    if (!empty($foundUnique)) {
                        $whereClause = ['where' => [$tablePrimaryKey => $foundUnique[$tablePrimaryKey]], 'data' => [$relatedForeignKey => null]];
                        $relationModel->update($whereClause);
                    }
                }
            } catch (\Exception $e) {
                throw new \Exception("Error disconnecting '{$relationName}': " . $e->getMessage());
            }
        }
    }
    ...
    {
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required when creating a new Tecnicos.");
        }
        ...
        foreach ($requiredFieldsMap as $fieldName => $fieldType) {
            if (!isset($data['data'][$fieldName])) {
                throw new \Exception("The '$fieldName' field of type '$fieldType' is required when creating a new Tecnicos.");
            }
        }
        ...
        $data = $data['data'];
        $relationNames = ['experimentos', 'pessoas'];
        ...
        try {
            ...
            $sql = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "INSERT INTO $quotedTableName ($fieldStr) VALUES ($placeholderStr) RETURNING $primaryKeyField" : "INSERT INTO $quotedTableName ($fieldStr) VALUES ($placeholderStr)";
            ...
            $stmt->execute();
            $lastInsertId = ($dbType == 'pgsql' || $dbType == 'sqlite') ? $stmt->fetch(\PDO::FETCH_ASSOC)[$primaryKeyField] : $this->_pdo->lastInsertId();
                ...
                foreach ($data as $relationName => $relationDataName) {
                    ...
                    if (in_array($relationName, $relationNames)) {
                        ...
                        if (isset($relationDataName['create']) || isset($relationDataName['createMany']) || isset($relationDataName['connect']) || isset($relationDataName['connectOrCreate'])) {
                            throw new \Exception("The relation name '$relationName' is not defined in the Tecnicos model.");
                        }
                    }
                }
            }
    ...
    public function findUnique(array $criteria, bool $format = false): array | object
    {
        ...
        $tablePrimaryKeys = ['matricula', 'registro'];
        $primaryEntityFields = [];
        $relatedEntityFields = [];
        $includes = [];
        ...
        if (!$whereHasUniqueKey) {
            throw new \Exception("No valid 'where' conditions provided for finding a unique record in Tecnicos.");
        }
        ...
        if (!empty($primaryEntityFields)) {
            $formattedFields = array_map(function ($field) use ($dbType) {
                return ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"$field\"" : "`$field`";
            }, $primaryEntityFields);
        ...
        Utility::queryOptions($criteria, $sql);
        ...
        foreach ($includes as $relation => $relationInclude) {
            if ($relationInclude) {
                ...
                if (method_exists($this, $includeMethodName)) {
                    ...
                    throw new \Exception("The '$relation' does not exist, in the Tecnicos model.");
                }
            }
        }
    ...
    public function findMany(array $criteria = [], bool $format = false): array | object
    {
        ...
        if (isset($criteria['cursor']) && is_array($criteria['cursor'])) {
            foreach ($criteria['cursor'] as $field => $value) {
                $select[$field] = ['>=' => $value];
                $fieldQuoted = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"$field\"" : "`$field`";
    ...
    {
        if (!isset($data['where'])) {
            throw new \Exception("The 'where' key is required in the update Tecnicos.");
        }
        ...
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required in the update Tecnicos.");
        }
        ...
        foreach ($requiredFieldsMap as $fieldName => $fieldType) {
            if (isset($data['data'][$fieldName]) && empty($data['data'][$fieldName])) {
                throw new \Exception("The '$fieldName' field of type '$fieldType' is required in Tecnicos Model.");
            }
        }
        ...
        try {
            ...
            foreach ($this->_fields as $field) {
                ...
                if (isset($data[$fieldName]) || !$isNullable) {
                    ...
                    $updateFields[] = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"$fieldName\" = :$fieldName" : "`$fieldName` = :$fieldName";
                    $bindings[":$fieldName"] = $validatedValue;
                } else {
                    if (array_key_exists($fieldName, $data) && $isNullable) {
                        if (isset($relation) || isset($inverseRelation) || isset($implicitRelation)) continue;
                        $updateFields[] = ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"$fieldName\" = NULL" : "`$fieldName` = NULL";
                    }
                }
            }
                ...
                foreach ($data as $relationName => $relationDataName) {
                    ...
                    if (in_array($relationName, $relationNames)) {
                        ...
                        if (isset($relationDataName['create']) && is_array($relationDataName['create'])) {
                            ...
                            if ($checkArrayContentType !== ArrayType::Associative) {
                                throw new \Exception("To create many records, use an associative array with the field names as keys and the data to create as values in pessoas model. ['createMany' => [['name' => 'someName'], ['name' => 'someOtherName']]");
                            }
                    ...
                    if (isset($relationDataName['connect']) || isset($relationDataName['connectOrCreate']) || isset($relationDataName['disconnect']) || isset($relationDataName['update']) || isset($relationDataName['updateMany'])) {
                        ...
                        if (isset($relationDataName['connect'])) {
                            ...
                            if (isset($relationDataName['connect']) && $checkArrayContentType !== ArrayType::Value) {
                                throw new \Exception("The 'connect' key must be an associative array with the field names as keys and the data to create as values related pessoas model. example: ['connect' => ['id' => 'someId']]");
                            }
                        }
                        ...
                        if (isset($relationDataName['connectOrCreate'])) {
                            ...
                            if (isset($relationDataName['connectOrCreate']) && $checkArrayContentType !== ArrayType::Associative) {
                                throw new \Exception("The 'connectOrCreate' key must be an associative array with the field names as keys and the data to create as values related pessoas model. example: ['connectOrCreate' => ['where' => ['id' => 'someId'], 'create' => ['name' => 'someName']]");
                            }
                        }
                        ...
                        if (isset($relationDataName['disconnect'])) {
                            ...
                            if (!is_bool($disconnectData)) {
                                ...
                                if (isset($relationDataName['disconnect']) && $checkArrayContentType !== ArrayType::Value) {
                                    throw new \Exception("The 'disconnect' key must be an associative array with the field names as keys and the data to create as values related pessoas model. example: ['disconnect' => ['id' => 'someId']]");
                                }
                            }
                        }
    ...
    public function delete(array $criteria, bool $format = false): array | object
    {
        ...
        try {
            ...
            return $affectedRows ? $deletedRow : ['modelName' => 'Tecnicos', 'cause' => 'Record to delete does not exist.'];
        } catch (\Exception $e) {
            $this->_pdo->rollBack();
            throw $e;
        }
    ...
    public function aggregate(array $operation, bool $format = false): array | object
    {
        ...
        $quotedTableName = $this->_dbType == 'pgsql' ? "\"tecnicos\"" : "`tecnicos`";
        $conditions = [];
        $bindings = [];
        ...
        Utility::queryOptions($operation, $subQuery);
     ...
     * @return array|object   An array or object containing the grouped data.
    ...
    {
        $acceptedCriteria = ['by', 'where', 'orderBy', 'skip', 'take', '_min', '_max', '_count'];
        Utility::checkForInvalidKeys($criteria, $acceptedCriteria, $this->_modelName);
        ...
        // Handle the case where the array contains a single element with multiple fields separated by commas
        if (is_array($by) && count($by) === 1 && strpos($by[0], ',') !== false) {
            $by = array_map('trim', explode(',', $by[0]));
        }

        // Quote fields for SQL
        $groupByFields = implode(', ', array_map(fn($field) => ($dbType == 'pgsql' || $dbType == 'sqlite') ? "\"$field\"" : "`$field`", $by));

        // Build the SQL query
        $sql = "SELECT $groupByFields FROM $quotedTableName";
        $conditions = [];
        $bindings = [];
    ...
    {
        if (!isset($data['where'])) {
            throw new \Exception("The 'where' key is required in the updateMany Tecnicos.");
        }
        ...
        if (!isset($data['data'])) {
            throw new \Exception("The 'data' key is required in the updateMany Tecnicos.");
        }
...
";
    ...
    public function deleteMany(array $criteria, bool $format = false): array | object 
    {
        ...
        try {
            ...
            // Process conditions if 'where' is set and not empty
    ...
    public function count(array $criteria = [], bool $format = false): int | array | object
    {
        ...
        $select = $criteria['select'] ?? [];
        $dbType = $this->_dbType;
        $quotedTableName = $dbType === 'pgsql' ? "\"tecnicos\"" : "`tecnicos`";
